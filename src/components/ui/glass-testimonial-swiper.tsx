...
export const TestimonialStack = ({ testimonials, visibleBehind = 2 }: TestimonialStackProps) => {
  ...
  return (
    <section className="relative pb-10 w-full">
      <style>{`
        .testimonial-card {
          position: absolute;
          width: 100%;
      ...
      <div className="pagination flex gap-2 justify-center">
        {testimonials.map((_, index) => (
          <button key={index} aria-label={`Go to item ${index + 1}`} onClick={() => navigate(index)} className={`pagination-dot ${activeIndex === index ? 'active' : ''}`} />
        ))}
      </div>
    </section>
  );
